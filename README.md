...
程序启动后默认隐藏主窗口，仅显示系统托盘图标；从托盘菜单可打开窗口或退出程序。“参数设置”页可以修改目录、排除规则、扩展名、数据库、事件参数、浏览器参数和应用进程映射。参数保存到 `config.json`，重启后生效。勾选“登录 Windows 后自动启动”并保存，会写入当前用户启动项，使用 `pythonw.exe` 静默启动，不需要管理员权限。

## 输入活动统计

启用后通过系统事件监听按小时汇总鼠标点击和键盘敲击次数。程序只保存两个计数，不保存具体按键、鼠标按钮、坐标或输入内容。监听是事件驱动的，不进行高频轮询；每小时最多写入两条数据库记录，也可以在参数设置页关闭。
